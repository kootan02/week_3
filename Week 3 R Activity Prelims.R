...
setwd("D:/Documents/McDaniel/ANA505")
getwd()
...
women_weight <- (new_women$weight)
head(women_weight)
women_height <- (new_women$height)
head(women_height)
...
data_weight <- new_women[,c("weight")]
data_height <- new_women[,c("height")]
data_weight
...
ggplot(new_women, aes(x=weight, y=height)) + 
  geom_point()

#Optional 'Just for fun' TASK: Change the arguments for the geom point and see what happens to the plot
ggplot(new_women, aes(x=height, y=weight)) + 
  geom_point(shape = 23, colour = "red", fill = "green", size = 3, stroke = 2)
